            ...
            GeometryReader { geometry in
                ScrollView([.vertical, .horizontal], showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                                ...
                                ForEach(Array(viewModel.columns.enumerated()), id: \.offset) { index, col in
                                    ...
                                    if let cell = cell {
                                        ...
                                        .background(GeometryReader { proxy in
                                            Color.clear.preference(key: PDFHeightPreferenceKey.self, value: [i: proxy.size.height])
                                        })
                                    }
                                }
                            }
                        ...
                        .onPreferenceChange(PDFHeightPreferenceKey.self) { value in
                    ...
                    .background( GeometryReader { geo in
                        Color.clear
                            .preference(key: PDFViewOffsetKey.self, value: geo.frame(in: .named("scroll")).origin)
                    })
                    .onPreferenceChange(PDFViewOffsetKey.self) { value in
                        offset = value
                    }
                }
                .onAppear {
...
struct PDFHeightPreferenceKey: PreferenceKey {
...
struct PDFViewOffsetKey: PreferenceKey {
