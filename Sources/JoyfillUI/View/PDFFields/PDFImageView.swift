...
//                    .padding(.bottom, 20)
            ...
            NavigationLink(destination: 
                            PDFMoreImageView(valueElements: $valueElements, isMultiEnabled: fieldDependency.fieldData?.multi ?? true, showToast: $showToast, uploadAction: uploadAction, isUploadHidden: fieldDependency.fieldPosition.primaryDisplayOnly ?? (fieldDependency.mode == .readonly))
...
struct PDFMoreImageView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            if !isUploadHidden {
                PDFUploadDeleteView(imagesArray: $images, selectedImages: $selectedImages,isMultiEnabled: $isMultiEnabled,valueElements: $valueElements, uploadAction: uploadAction, deleteAction: deleteSelectedImages)
            }
            if showProgressView {
                ProgressView()
            }else {
        ...
        .overlay(content: {
            if showToast {
                VStack {
                       Spacer()
                       PDFToastMessageView(message: "Image is already uploaded", duration: 2.0, isPresented: $showToast)
                           .padding(.top, 50)
...
struct PDFUploadDeleteView: View {
...
struct PDFToastMessageView: View {
    let message: String
