...
/// Protocol adopted by host applications to observe Joyfill form events.
public protocol FormChangeEvent {

    /// Called whenever one or more fields change.
    /// - Parameters:
    ///   - changes: Collection of changelog entries describing the update.
    ///   - document: Updated JoyDoc snapshot after the change was applied.
    func onChange(changes: [Change], document: JoyDoc)

    /// Called when a field gains focus.
    /// - Parameter event: Identifier describing the field that gained focus.
    func onFocus(event: FieldIdentifier)

    /// Called when a field loses focus.
    /// - Parameter event: Identifier describing the field that lost focus.
    func onBlur(event: FieldIdentifier)

    /// Called when an upload operation completes.
    /// - Parameter event: Payload describing the upload context.
    func onUpload(event:UploadEvent)

    /// Called when a capture target (e.g. table cell, nested collection) requests user input.
    /// - Parameter event: Payload describing the capture request and completion handler.
    func onCapture(event: CaptureEvent)

    /// Called when a Joyfill UI error is encountered.
