func items(context: JSONSchemaContext, items: Any, instance: Any, schema: [String: Any]) throws -> AnySequence<ValidationError> {
  ...
  if let items = items as? [String: Any] {
    if !context.collectAllErrors {
      for (index, item) in instances {
        ...
        let result = try context.descend(instance: item, subschema: items)
  ...
  if let items = items as? [Any] {
    ...
    for (index, item) in instances where index < items.count {
      context.instanceLocation.push(index.description)
      defer { context.instanceLocation.pop() }
      let result = try context.descend(instance: item, subschema: items[index])
      if !context.collectAllErrors && !result.isValid {
        return result
      }
