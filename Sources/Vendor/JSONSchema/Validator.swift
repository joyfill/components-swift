...
class Context {
  ...
  /// When `false`, `validate(instance:schema:)` stops after the first failing
  /// keyword instead of collecting all errors.  Set to `false` by `anyOf` /
  /// `not` before probing each alternative — we only need to know *whether*
  /// the alternative is valid, not *why* it failed.
  var collectAllErrors: Bool = true
  ...
  func validate(instance: Any, schema: Any) throws -> AnySequence<ValidationError> {
    ...
    // Fast path: stop at first failing keyword (used by anyOf / not probes).
    if !collectAllErrors {
      for (key, validation) in validator.validations {
        guard let value = schema[key] else { continue }
        keywordLocation.push(key)
        let result = try validation(self, value, instance, schema)
        keywordLocation.pop()
        if !result.isValid {
          return result
        }
