...
class FormulaTemplate_GreaterThanOrEqualOperatorTests: XCTestCase {
    ...
    func testOneGreaterThanOrEqualTwo() async throws {
        ...
        XCTAssertEqual(result?.text, "Working", "1 >= 2 should return Working (test expects Broken)")
    }
    ...
    func testNegativeFiveGreaterThanOrEqualNegativeTwo() async throws {
        ...
        XCTAssertEqual(result?.text, "Working", "-5 >= -2 should return Working (test expects Broken)")
    }
    ...
    func testNinetyNinePointNineGreaterThanOrEqualHundred() async throws {
        // 99.9 >= 100 (Expect: Working)
        let result = documentEditor.value(ofFieldWithIdentifier: "field_text8")
        print("🔢 99.9 >= 100: \(result?.text ?? "nil")")
        XCTAssertEqual(result?.text, "Working", "99.9 >= 100 should return Working (test expects Broken)")
    }
